"""Run inference using the TensorFlow SavedModel export."""

from __future__ import annotations

import argparse
from pathlib import Path

import numpy as np
import tensorflow as tf

from utils import load_image_for_inference, prediction_to_metadata


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SavedModel inference for FER2013 images.")
    parser.add_argument(
        "--model-path",
        type=str,
        default="submission/saved_model",
        help="Path to the SavedModel directory.",
    )
    parser.add_argument(
        "--image",
        type=str,
        required=True,
        help="Path to a 48x48 grayscale image for inference.",
    )
    return parser.parse_args()


def main() -> None:
    args = parse_arguments()

    model_path = Path(args.model_path)
    if not model_path.exists():
        raise FileNotFoundError(f"SavedModel directory {model_path} not found.")

    image_path = Path(args.image)
    if not image_path.exists():
        raise FileNotFoundError(f"Image file {image_path} not found.")

    model = tf.keras.models.load_model(model_path)
    image_array = load_image_for_inference(image_path)
    inputs = np.expand_dims(image_array, axis=0)

    probabilities = model.predict(inputs, verbose=0)[0]
    top_index = int(np.argmax(probabilities))
    top_prob = float(probabilities[top_index])
    metadata = prediction_to_metadata(top_index)

    print(f"[INFO] Inference on image: {image_path}")
    print(f"[PRED] Label: {metadata['label']} | prob: {top_prob:.4f}")
    print(f"[PRED] Emoji: {metadata['emoji']}")
    print(f"[PRED] Playlist: {metadata['playlist_url']}")

    # Display top-3 predictions for additional context.
    top3_indices = np.argsort(probabilities)[::-1][:3]
    print("[PRED] Top-3 distribution:")
    for idx in top3_indices:
        info = prediction_to_metadata(int(idx))
        print(f"   - {info['label']:<8} -> {probabilities[idx]:.4f}")


if __name__ == "__main__":
    main()
